...
public class DateTimeUtils {
    ...
    public static String javaDateToTimeSpan(final Date date) {
        ...
        if (secondsSince < 60)
            return CMS.getContext().getString(R.string.reader_timespan_now);
        ...
        if (daysSince < 365)
            return DateUtils.formatDateTime(CMS.getContext(), passedTime, DateUtils.FORMAT_NO_YEAR | DateUtils.FORMAT_ABBREV_ALL);

        // date is older, so include year (ex: Jan 30, 2013)
        return DateUtils.formatDateTime(CMS.getContext(), passedTime, DateUtils.FORMAT_ABBREV_ALL);
    }
