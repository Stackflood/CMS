...
import com.example.manish.androidcms.models.FeatureSet;
...
public class PostUploadService extends Service {
    ...
    public static void addPostToUpload(Post currentPost) {
        synchronized (mPostsList) {
            ...
            CMS.cmsDB.updatePost(currentPost);
        }
    }
    ...
    public void onDestroy() {
        super.onDestroy();
        // Cancel current task,
        // it will reset post from "uploading" to "local draft"
    ...
    private FeatureSet synchronousGetFeatureSet() {
        if (CMS.getCurrentBlog() == null || !CMS.getCurrentBlog().isDotcomFlag()) {
            return null;
        }
    ...
    private class UploadPostTask extends AsyncTask<Post, Boolean, Boolean> {
        ...
        protected void onPostExecute(Boolean postUploadedSuccessfully) {
            ...
            if (postUploadedSuccessfully) {
                CMS.postUploaded(mPost.getLocalTableBlogId(),
                        mPost.getRemotePostId(),
                        mPost.isPage());
                mPostUploadNotifier.cancelNotification();
                CMS.cmsDB.deleteMediaFilesForPost(mPost);
            } else {
                CMS.postUploadFailed(mPost.getLocalTableBlogId());
                mPostUploadNotifier.updateNotificationWithError(mErrorMessage, mIsMediaError,
                        mPost.isPage(),
        ...
        protected void onCancelled(Boolean aBoolean) {
            ...
            if (mPostUploadNotifier != null && mPost != null) {
                mPostUploadNotifier.updateNotificationWithError(mErrorMessage,
                        mIsMediaError, mPost.isPage(),
                        mErrorUnavailableVideoPress);
                CMS.postUploadFailed(mPost.getLocalTableBlogId());
            }
        }
        ...
        protected Boolean doInBackground(Post... posts) {
            ...
            mPostUploadNotifier.updateNotificationMessage(uploadingPostTitle,
                    uploadingPostMessage);

            mBlog = CMS.cmsDB.instantiateBlogByLocalId(mPost.getLocalTableBlogId());
            if (mBlog == null) {
            ...
            contentStruct.put((mPost.isPage()) ? "page_status" : "post_status",
                    mPost.getPostStatus());
            ...
            if (mPost.isLocalDraft() && !mPost.isUploaded())
                params = new Object[]{mBlog.getRemoteBlogId(),
                        mBlog.getUsername(), mBlog.getPassword(),
                        contentStruct, false};
            else
                params = new Object[]{mPost.getRemotePostId(),
                        mBlog.getUsername(), mBlog.getPassword(), contentStruct,
                        false};
        ...
        private String processPostMedia(String postContent) {
            ...
            for (String tag : imageTags) {
                ...
                if (m.find()) {
                    ...
                    if (!imageUri.equals("")) {
                        MediaFile mediaFile = CMS.cmsDB.getMediaFile(imageUri, mPost);
                        if (mediaFile != null) {
        ...
        private String uploadImage(MediaFile mediaFile) {
            ...
            if (shouldUploadResizedVersion) {
                ...
                byte[] bytes = ImageUtils.createThumbnailFromUri(mContext,
                        imageUri, resizedMediaFile.getWidth(),
                ...
                if (bytes == null) {
                    ...
                    if (!TextUtils.isEmpty(tempFilePath)) {
                        ...
                        resizedPictureURL = uploadImageFile(parameters,
                                resizedMediaFile, mBlog);
            ...
            return mediaFile.getImageHtmlForUrls(fullSizeUrl,
                    resizedPictureURL, shouldAddImageWidthCSS);
        }
        ...
        private String uploadVideo(MediaFile mediaFile) {
            ...
            if (videoUri.toString().contains("content:")) {
                ...
                if (cur != null && cur.moveToFirst()) {
                    ...
                    int mimeTypeColumn = cur.getColumnIndex(MediaStore.Video.Media.MIME_TYPE);
            ...
            boolean selfHosted = CMS.currentBlog != null && !CMS.currentBlog.isDotcomFlag();
    ...
    private class PostUploadNotifier {
        ...
        public void updateNotificationWithError(String mErrorMessage, boolean isMediaError, boolean isPage,
            ...
            AppLog.d(AppLog.T.POSTS, "updateNotificationWithError: " + mErrorMessage);
