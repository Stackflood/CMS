...
public class CategoryNode {
    ...
    public static CategoryNode createCategoryTreeFromDB(int blogId) {
        ...
        List<String> stringCategories = CMS.cmsDB.loadCategories(blogId);
        ...
        for (String name : stringCategories) {
             int categoryId = CMS.cmsDB.getCategoryId(blogId, name);
            int parentId = CMS.cmsDB.getCategoryParentId(blogId, name);
        ...
        for(int i = 0; i < categoryMap.size(); i++){
            ...
            currentRootNode.children.put(category.getName(),
                    categoryMap.get(category.getCategoryId()));
        }
        return rootCategory;
    }

    private static void preOrderTreeTraversal(CategoryNode node,
                                              int level,
                                              ArrayList<CategoryNode> returnValue) {
        if (node == null) {
            return ;
        }
        if (node.parentId != -1) {
