...
import com.example.manish.androidcms.models.BlogIdentifier;
...
import com.example.manish.androidcms.ui.media.MediaGridFragment.Filter;
import com.example.manish.androidcms.ui.posts.PostsListFragment;
...
public class ApiHelper {
    ...
    public static synchronized void updateBlogOptions(Blog currentBlog, Map<?, ?> blogOptions) {
        ...
        if (isModified) {
            CMS.cmsDB.saveBlog(currentBlog);
        }
    }
        ...
        protected Boolean doInBackground(List<?>... params) {
            ...
            try {
                ...
                if (result != null && result.length > 0) {
                    ...
                    CMS.cmsDB.savePosts(postsList, blog.getLocalTableBlogId(), isPage, !loadMore);
                }
                return true;
            } catch (XMLRPCFault e) {
        ...
        protected Boolean doInBackground(List<?>... params) {
            ...
            try {
                ...
                if (result != null && result instanceof Map) {
                    ...
                    CMS.cmsDB.savePosts(postsList, blog.getLocalTableBlogId(), isPage, true);
                }
    ...
    public static class SyncMediaLibraryTask extends HelperAsyncTask<java.util.List<?>, Void, Integer> {
        ...
        protected Integer doInBackground(List<?>... params) {
            List<?> arguments = params[0];
            CMS.currentBlog = (Blog) arguments.get(0);
            ...
            if (mOffset == 0) {
                //CMS.cmsDB.setMediaFilesMarkedForDeleted(blogId);
            }
            for (Object result : results) {
                resultMap = (Map<?, ?>) result;
                boolean isDotCom = (CMS.getCurrentBlog() != null && CMS.getCurrentBlog().isDotcomFlag());
...
public class ApiHelper {
    ...
    public static abstract class HelperAsyncTask<Params, Progress, Result> extends AsyncTask<Params,
            Progress, Result> {
        protected String mErrorMessage;
        protected ErrorType mErrorType = ErrorType.NO_ERROR;
        protected Throwable mThrowable;
...
/*");
            ...
            //CMS.cmsDB.deleteFilesMarkedForDeleted(blogId);
            return results.length;
        }
...
/**
    ...
    private static String getResponse(final String url, int maxRedirection)
        ...
        CMS.requestQueue.add(stringRequest);
        try {
